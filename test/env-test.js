...
const root = "F:\\GitHub\\_node\\xtrmaddons-soft-suite-conf";
	...
	["host", "port", "protocol"].forEach(param => {
		["argv", "file", "default"].forEach(from => {
			if(Config.get(param) == server[from][param])
				it(`Config.get(${param}) from '${from}' value "${server[from][param]}"`, function() {
...
describe("Config Custom properties Mocha String Test", function () {
	Config.set("custom:var1", "test property 1");
	Config.save();
	it("Config.get('custom:var1')  should be test property 1", function() {
		assert.equal(Config.get("custom:var1") , "test property 1");
	});
});
